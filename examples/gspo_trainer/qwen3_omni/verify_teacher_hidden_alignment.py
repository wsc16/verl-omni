...
End-to-end check of the nitrobrew premise for Qwen3-Omni thinker:
    ...
    2. Send a request with ``return_hidden_states`` enabled.
...
"""
    ...
    try:
        ...
        hidden = extract_hidden_states(final)
        assert hidden is not None, (
            f"No {TEACHER_HIDDEN_STATES_KEY} in output; mm channel did not carry the payload"
        )
        return hidden.float()
    finally:
        engine.shutdown()
